...
int publisher_main(int domainId, int sample_count)
{
    ...
    /** Function to create domain participant through QOS Profiles 
    set on USER_QOS_PROFILES.xml */
    ...
    from the created domain participant instance */
...
//for read data
        ...
        //instance=999;
        //RTI_SNPRINTF(instance,10,"%d",Usage_L);
        //float Watts=5;
        FILE *file;
        char buffer[BUFSIZ];
        int n = 0;
        ...
        //{
            ...
            instance-> SendTime=tolong*1000+current.nanosec/1000000;     
                ...
                l=lines[1];
                l=l->next;
                instance-> Hot_Valve=atol(l->s);
                

                l=lines[2];
                l=l->next;
                instance-> Cooling=atol(l->s);
                
                
                l=lines[3];
                l=l->next;
                instance-> Usage_L=atol(l->s);
                
                l=lines[4];
                l=l->next;
                instance-> Warm_Valve=atol(l->s);
                ...
                l=lines[15];
                l=l->next;
                instance-> Heating=atol(l->s);

                l=lines[16];
                l=l->next;
                instance-> Cold_Valve=atol(l->s);
                
                l=lines[17];
                l=l->next;
                instance-> WarmTemp=atol(l->s);

                l=lines[18];
                l=l->next;
                instance-> ColdTemp=atol(l->s);

                l=lines[19];
                l=l->next;
                instance-> Usage_CC=atol(l->s);
                
                l=lines[20];
                l=l->next;
                instance-> HotTemp=atol(l->s);
            ...
            //if(l==NULL) printf("NO");
            //printf("%s", lines[2][0]?:'-');
            int total = instance-> Usage_CC+1000*(instance-> Usage_L)+1000000*(instance-> Usage_MT);
        fclose (file);
        ////
        typedef struct{
                    char* ss;
        ...
        int c1;
        //const char * const delim = ",";
        // FILE *file0 = fopen("/home/GYM0202/DDS/field_seldom/DDS_Data_old.csv", "w"); 
        ...
        file1 = fopen("/home/T20501/DDS/dispenser_pi/DDS_Data_old.csv", "r");
        if (!file1) {
                        perror("error to open file");
                        return EXIT_FAILURE;
                }
        ...
        while (fgets(line1, line_size1, file1) != NULL)  {
            ...
            ll=lines1[0];
            ll=ll->next1;
            instance1-> Usage_CC=atol(ll->ss);

            ll=lines1[1];
            ll=ll->next1;
            instance1-> Usage_L=atol(ll->ss);

            ll=lines1[2];
            ll=ll->next1;
            instance1-> Usage_MT=atol(ll->ss);

            int total1 = instance1-> Usage_CC+1000*(instance1-> Usage_L)+1000000*(instance1-> Usage_MT);
            // int ColdTemp_Insulation=instance1-> ColdTemp_Insulation;
            // int HotTemp_Insulationcompare=instance1-> HotTemp_Insulation;
            printf("%s,%d\n","total1",total1);  
        fclose (file1);
            
        if ((total>=total1) && (((total-total1)<800)||((total-total1)>1200)) ){
        ...
        /** Write data */
            ...
            FILE *file2 = fopen("/home/T20501/DDS/dispenser_pi/DDS_Data_old.csv", "w"); 
            if (file2 == NULL) return -1; 
            //while (count-- > 0) { 
                // you might want to check for out-of-disk-space here, too 
            fprintf(file2, "%s,%d\n","Usage_CC",instance-> Usage_CC); 
            fprintf(file2, "%s,%d\n","Usage_L",instance-> Usage_L); 
