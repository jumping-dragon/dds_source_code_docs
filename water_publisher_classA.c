...
//for read data
       ...
       typedef struct{
        ...
        file1 = fopen("/home/T20501/DDS/field_always/DDS_Data_old.csv", "r");
        if (!file1) {
                        perror("error to open file");
                        return EXIT_FAILURE;
                }
        ...
        while (fgets(line1, line_size1, file1) != NULL)  {
            ...
            ll=lines1[0];
            ll=ll->next1;
            instance1-> WarmTemp=atol(ll->ss);//Warm_Valve=atol(ll->ss);

            ll=lines1[1];
            ll=ll->next1;
            instance1-> ColdTemp=atol(ll->ss);//Refilling=atol(ll->ss);

            ll=lines1[2];
            ll=ll->next1;
            instance1-> Usage_CC=atol(ll->ss); //Heating=atol(ll->ss);

            ll=lines1[3];
            ll=ll->next1;
            instance1-> HotTemp=atol(ll->ss);//Cold_Valve=atol(ll->ss);

            ll=lines1[4];
            ll=ll->next1;
            instance1-> TDS=atol(ll->ss); 
        ...
        if ((instance-> WarmTemp != instance1-> WarmTemp)||(instance-> ColdTemp != instance1-> ColdTemp)||(instance-> Usage_CC != instance1-> Usage_CC)||(instance-> HotTemp != instance1-> HotTemp)||(instance-> TDS != instance1-> TDS))
        {
        /* Write data */
            ...
            FILE *file2 = fopen("/home/T20501/DDS/field_always/DDS_Data_old.csv", "w"); 
            if (file2 == NULL) return -1; 
            //while (count-- > 0) { 
                // you might want to check for out-of-disk-space here, too 
  
            fprintf(file2, "%s,%d\n","WarmTemp",instance-> WarmTemp); 
            fprintf(file2, "%s,%d\n","ColdTemp",instance-> ColdTemp); 
            fprintf(file2, "%s,%d\n","Usage_CC",instance-> Usage_CC); 
            fprintf(file2, "%s,%d\n","HotTemp",instance-> HotTemp); 
            fprintf(file2, "%s,%d\n","TDS",instance-> TDS); 
