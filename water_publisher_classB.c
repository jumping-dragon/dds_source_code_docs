...
int publisher_main(int domainId, int sample_count)
{
    ...
    writer = DDS_Publisher_create_datawriter_with_profile(
        publisher, topic,
        "TCP_Library", "TCP_Publisher", NULL /* listener */, DDS_STATUS_MASK_NONE);
    if (writer == NULL) {
...
//for read data
       ...
       typedef struct{
        ...
        file1 = fopen("/home/T20501/DDS/field_regular/DDS_Data_old.csv", "r");
        if (!file1) {
                        perror("error to open file");
                        return EXIT_FAILURE;
                }
        ...
        while (fgets(line1, line_size1, file1) != NULL)  {
            ...
            ll=lines1[0];
            ll=ll->next1;
            instance1-> Hot_Valve=atol(ll->ss);

            ll=lines1[1];
            ll=ll->next1;
            instance1-> Cooling=atol(ll->ss);
            ...
            ll=lines1[3];
            ll=ll->next1;
            instance1-> Warm_Valve=atol(ll->ss);

            ll=lines1[4];
            ll=ll->next1;
            instance1-> Refilling=atol(ll->ss);

            ll=lines1[5];
            ll=ll->next1;
            instance1-> Heating=atol(ll->ss);

            ll=lines1[6];
            ll=ll->next1;
            instance1-> Cold_Valve=atol(ll->ss);
        ...
        if ((instance-> Cold_Valve != instance1-> Cold_Valve)||(instance-> Warm_Valve != instance1-> Warm_Valve)||(instance-> Hot_Valve != instance1-> Hot_Valve)||(instance-> Heating != instance1-> Heating)||(instance-> Cooling != instance1-> Cooling)||(instance-> Refilling != instance1-> Refilling)||(instance-> Usage_L != instance1-> Usage_L))
        {
        /* Write data */
            ...
            FILE *file2 = fopen("/home/T20501/DDS/field_regular/DDS_Data_old.csv", "w"); 
            if (file2 == NULL) return -1; 
            //while (count-- > 0) { 
                // you might want to check for out-of-disk-space here, too 
            fprintf(file2, "%s,%d\n","Hot_Valve",instance-> Hot_Valve); 
            fprintf(file2, "%s,%d\n","Cooling",instance-> Cooling); 
            fprintf(file2, "%s,%d\n","Usage_L",instance-> Usage_L); 
            fprintf(file2, "%s,%d\n","Warm_Valve",instance-> Warm_Valve); 
            fprintf(file2, "%s,%d\n","Refilling",instance-> Refilling); 
            fprintf(file2, "%s,%d\n","Heating",instance-> Heating);   
            fprintf(file2, "%s,%d\n","Cold_Valve",instance-> Cold_Valve);    
            //} Hot_Valve,0
